...
PREFIX="${NGSPICE_PREFIX:-/opt/ngspice}"
WORK_DIR="${NGSPICE_BUILD_DIR:-/tmp/bluespice-ngspice-build}"
TARBALL="${WORK_DIR}/ngspice-${VERSION}.tar.gz"
SRC_DIR="${WORK_DIR}/ngspice-${VERSION}"

if [[ -f "${PREFIX}/lib/libngspice.so" ]]; then
  echo "ngspice shared library already exists at ${PREFIX}/lib/libngspice.so"
  exit 0
fi
